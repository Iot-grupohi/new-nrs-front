(() => {
  ...
  function isSupportRow(row) {
    ...
    return page === 'suporte' || action.startsWith('support_');
  }

  function supportMeta(row) {
    const meta = row?.meta && typeof row.meta === 'object' ? row.meta : {};
    return {
      categoryId: String(meta.category_id || '').trim(),
      procedureId: String(meta.procedure_id || '').trim(),
    };
  }
  ...
  function equipmentType(row) {
    if (isSupportRow(row)) return 'RUNBOOK';
  ...
  function equipmentCode(row) {
    if (isSupportRow(row)) {
      const { categoryId, procedureId } = supportMeta(row);
      if (categoryId && procedureId) return `${categoryId}/${procedureId}`;
  ...
  function channelLabel(row) {
    ...
    if (page === 'suporte') return CHANNEL_LABELS.suporte;
    return '—';
  }

  function operatorCellText(row) {
    const email = String(row?.operator_email || row?.user_email || '').trim();
    if (email) return email;
    const action = String(row?.action || '');
    if (action === 'auth_login_failed') return 'Não identificado';
    const name = String(row?.operator_name || '').trim();
    if (name && name !== 'Operador desconhecido') return name;
    return '—';
  }
  ...
  function actionCellText(row) {
    ...
    if (action === 'auth_login_failed') return 'Login recusado';
  ...
  function visibleOperatorStats() {
    return operatorStats.filter((row) => {
      const email = String(row?.email || '').trim().toLowerCase();
      const name = String(row?.name || '').trim();
      if (!email || email === '__unknown__') return false;
      if (name === 'Operador desconhecido') return false;
      return true;
    });
  }

  function renderOperatorStats() {
    const stats = visibleOperatorStats();
    ...
    const maxCount = Math.max(...stats.map((row) => Number(row.count) || 0), 1);

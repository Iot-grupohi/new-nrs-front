...
from lav60_env import configure_frozen_tls
...
from lav60_env import load_local_env, env_file_path
from lav60_firebase_rtdb import firebase_rtdb_enabled, firebase_rtdb_status, send_firebase_rtdb_heartbeat
...
DRYER_RELEASE_PULSE_INTERVAL = float(os.getenv('DRYER_RELEASE_PULSE_INTERVAL', '1'))
...
def panel_heartbeat_post_enabled() -> bool:
    """POST /api/heartbeat legado — desligado por padrão quando RTDB está ativo."""
    raw = (
        os.getenv('PANEL_HEARTBEAT_POST_ENABLED')
        or _windows_env('PANEL_HEARTBEAT_POST_ENABLED')
        or ''
    ).strip().lower()
    ...
    if raw in {'1', 'true', 'yes', 'on'}:
        return True
    return not firebase_rtdb_enabled()
...
def _agent_env_source(name: str) -> str:
    """Origem da variável: registro Windows, .env ou ambiente do processo."""
    if os.name == 'nt' and _read_windows_registry_env(name):
        return 'registro Windows'
    if (os.environ.get(name) or '').strip():
        return '.env' if load_local_env() else 'variável de ambiente'
    return ''
...
def require_store_id() -> str:
    ...
    raise RuntimeError(
        'Defina STORE_ID no registro Windows (variáveis de ambiente do usuário ou do sistema). '
        'Reinicie o agente após alterar.'
    )


def env_store_id() -> str:
    """Código da loja — obrigatório via STORE_ID (ambiente / .env / registro Windows)."""
...
def process_release_async(machine_id: str, target_url: str, release_count: int, params: dict, request_id: str, start_time: float):
    ...
    try:
        ...
        for i in range(release_count):
            ...
            # Intervalo entre pulsos (exceto após o último)
            if i < release_count - 1:
                logger.info(
                    f"[{request_id}] ⏳ Waiting {DRYER_RELEASE_PULSE_INTERVAL}s before next request..."
                )
                time.sleep(DRYER_RELEASE_PULSE_INTERVAL)
...
def process_dryer_remaining_releases(
    ...
    try:
        for i in range(start_index, release_count):
            time.sleep(DRYER_RELEASE_PULSE_INTERVAL)
            logger.info(
                f"[{request_id}] 📤 Dryer {machine_id} release {i + 1}/{release_count} "
                f"(background, +{DRYER_RELEASE_PULSE_INTERVAL}s)"
            )
            try:
...
def get_store_machines_list() -> list[dict]:
    ...
    if not snap:
        return []
    machines = list(snap.get('machines') or [])
    # API sem dosadoras (ex.: PB11): incluir mapa local no catálogo/heartbeat.
    if not snap.get('doser_map'):
        existing = {
            ('doser', normalize_machine_id(str(machine.get('id', ''))))
            for machine in machines
            if canonical_device_type(machine.get('type', '')) == 'doser'
        }
        for mid, ip in DOSADORA_MAP.items():
            key = ('doser', normalize_machine_id(mid))
            if key not in existing:
                machines.append(_build_local_doser_machine(mid, ip))
    machines.sort(key=lambda m: (m.get('type', ''), m.get('id', '')))
    return machines
...
def _build_local_doser_machine(machine_id: str, ip: str) -> dict:
    address = normalize_device_address(ip)
    return {
        'api_id': '',
        'resource_type': 'machine',
        'id': normalize_machine_id(machine_id),
        'type': 'doser',
        'machine_type': 'doser',
        'machine_type_label': MACHINE_TYPE_LABELS['doser'],
        'address': address,
        'status': 'available',
        'status_raw': 'available',
        'status_label': machine_status_label('available'),
        'capacity': '—',
        'capacity_raw': '',
        'liter_capacity': None,
        ...
        'time_dosage': None,
        'port': None,
        'store_code': env_store_id(),
        'endpoints': {'status': device_http_url(address, '/status')} if address else {},
        'source': 'local_map',
    }
...
def enrich_machines_for_frontend(machines: list[dict], network: dict | None = None) -> list[dict]:
    """Inclui online da rede e status operacional que o painel exige nos cards."""
    net = network if network is not None else get_cached_network_status()
    online_maps = {
        'washer': (net or {}).get('washers') or {},
        'dryer': (net or {}).get('dryers') or {},
        'doser': (net or {}).get('dosers') or {},
    }
    enriched: list[dict] = []
    for machine in machines or []:
        m = dict(machine)
        dtype = canonical_device_type(m.get('type', '')) or str(m.get('type', '')).strip().lower()
        mid = normalize_machine_id(str(m.get('id', '')))
        online_map = online_maps.get(dtype, {})
        if mid in online_map:
            m['online'] = bool(online_map[mid])
        if dtype == 'doser':
            current = normalize_api_machine_status(m.get('status'))
            if current != 'suspended' and (
                current in ('', 'unknown') or m.get('source') == 'local_map'
            ):
                m['status'] = 'available'
                m['status_raw'] = 'available'
                m['status_label'] = machine_status_label('available')
        enriched.append(m)
    return enriched


def machines_for_frontend(machines: list[dict] | None = None, network: dict | None = None) -> list[dict]:
    base = machines if machines is not None else get_store_machines_list()
    visible = filter_machines_for_frontend(base, network)
    return enrich_machines_for_frontend(visible, network)
...
def send_immediate_rtdb_heartbeat() -> bool:
    """Publica heartbeat no RTDB assim que o agente sobe (antes da leitura de rede)."""
    if not firebase_rtdb_enabled():
        return False
    store = require_store_id()
    payload = {
        'store': store.lower(),
        'agent_url': agent_static_public_url(),
        'agent_local_url': f'http://127.0.0.1:{SERVER_PORT}',
        'timestamp': datetime.now().isoformat(),
        'lav60_status': store_lav60_api_status or 'starting',
        'network': None,
        'machines': [],
    }
    try:
        ok = send_firebase_rtdb_heartbeat(store, payload)
        if ok:
            logger.info(
                f"{Fore.GREEN}🔥 Firebase RTDB heartbeat inicial → /{store.upper()}{Style.RESET_ALL}"
            )
        return ok
    except Exception as exc:
        logger.warning(f"{Fore.YELLOW}🔥 Firebase RTDB heartbeat inicial: {exc}{Style.RESET_ALL}")
        return False
...
def send_panel_heartbeat_once() -> bool:
    ...
    if firebase_rtdb_enabled():
        try:
            rtdb_ok = bool(send_firebase_rtdb_heartbeat(require_store_id(), payload))
        except Exception as exc:
            logger.warning(f"{Fore.YELLOW}🔥 Firebase RTDB heartbeat: {exc}{Style.RESET_ALL}")

    if not panel_heartbeat_post_enabled():
        if rtdb_ok:
            return True
        if firebase_rtdb_enabled():
            logger.warning(
                f"{Fore.YELLOW}🔥 Firebase RTDB heartbeat falhou — painel sem pulso da loja "
                f"{require_store_id().upper()}{Style.RESET_ALL}"
            )
        return False
...
def monitor_panel_heartbeat():
    ...
    if panel_heartbeat_post_enabled():
        logger.info(
            f"{Fore.CYAN}💓 Heartbeat painel POST (tentativas: "
            f"{', '.join(get_panel_heartbeat_url_candidates())}) "
            f"a cada {HEARTBEAT_INTERVAL}s{Style.RESET_ALL}"
        )
    else:
        logger.info(
            f"{Fore.CYAN}💓 Heartbeat via Firebase RTDB "
            f"(/{require_store_id().upper()}/gateway_heartbeat) a cada {HEARTBEAT_INTERVAL}s "
            f"— POST /api/heartbeat desativado{Style.RESET_ALL}"
        )
    send_panel_heartbeat_once()
    if wait_for_initial_network_status():
        logger.info(
            f"{Fore.CYAN}💓 Dispositivos lidos — reenviando heartbeat com status da rede{Style.RESET_ALL}"
        )
        send_panel_heartbeat_once()
    else:
        logger.warning(
...
def gateway_status_payload(store: str, machine: str | None, device_type: str | None):
    ...
    if not machine and not device_type:
        ...
        data['machines'] = machines_for_frontend(network=raw)
...
def agent_api_config():
    ...
    return jsonify({
        ...
        'machines': machines_for_frontend(network=cached_raw),
...
def ensure_tunnel_and_config(tunnel_name: str, config_path: str, rewrite_config: bool = True) -> str | None:
    ...
    try:
        new_config = (
            ...
            f"ingress:\n"
            f"  - hostname: {hostname}\n"
            f"    service: http://localhost:{SERVER_PORT}\n"
...
def build_debug_snapshot() -> dict:
    ...
    return {
        ...
        'firebase_rtdb': firebase_rtdb_status(),
    }
...
def run_server() -> None:
    ...
    if not check_and_cleanup_existing_instances():
        ...
        logger.error(f"{Fore.RED}❌ Could not release port {SERVER_PORT}. Check for other processes using the port{Style.RESET_ALL}")
        fatal_exit(1, f'Porta {SERVER_PORT} em uso. Feche o outro programa ou altere SERVER_PORT.')
    ...
    try:
        ...
        store_src = _agent_env_source('STORE_ID') or 'desconhecida'
        logger.info(
            f"{Fore.CYAN}🏪 STORE_ID: {Fore.WHITE}{store_id}{Style.RESET_ALL} "
            f"(fonte: {store_src})"
        )
        ...
        if env_file:
            ...
            logger.warning(
                f"{Fore.YELLOW}⚠️ .env não encontrado em {expected} — "
                f"copie .env.example para .env e configure tokens/Firebase{Style.RESET_ALL}"
            )
    ...
    if firebase_rtdb_enabled():
        fb = firebase_rtdb_status()
        logger.info(
            f"{Fore.CYAN}🔥 Firebase RTDB heartbeat: {Fore.WHITE}{fb.get('database_url')} "
            f"(loja /{store_id.upper()}){Style.RESET_ALL}"
        )
        send_immediate_rtdb_heartbeat()
    else:
        fb = firebase_rtdb_status()
        reason = fb.get('reason') or 'variáveis Firebase ausentes no .env'
        logger.info(f"{Fore.YELLOW}🔥 Firebase RTDB heartbeat: desativado ({reason}){Style.RESET_ALL}")

    log_section("STARTING PROXY SERVER", "INFO")
    logger.info(f"{Fore.CYAN}🌐 Server: {Fore.WHITE}http://0.0.0.0:{SERVER_PORT}{Style.RESET_ALL}")
